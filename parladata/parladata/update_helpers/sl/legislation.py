...
def set_legislation_result():
    for legislation in Law.objects.exclude(
        status__in=LegislationStatus.objects.filter(name__in=["rejected", "enacted"])
    ):
...
def set_single_legislation_result(legislation):
    if legislation.text:
        final_vote = Motion.objects.filter(law=legislation, title__icontains="v celoti")
        repeated_vote = Motion.objects.filter(
            law=legislation, title__icontains="ponovno odlo"
        )
        mdt_vote = Motion.objects.filter(law=legislation, title__icontains="sklep mdt")
        beginning_vote = Motion.objects.filter(
            law=legislation, title__icontains="sklep o primernosti predloga zakona"
        )
