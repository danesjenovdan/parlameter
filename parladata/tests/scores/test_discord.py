...
from parlacards.scores.discord import calculate_group_discord, calculate_organization_vote_discord
...
def test_calculate_organization_vote_discord(
    ...
    # group
    discord = calculate_organization_vote_discord(first_vote, first_group, main_organization, main_organization, None)
    assert discord == 100

    discord =  calculate_organization_vote_discord(first_vote, second_group, main_organization, main_organization, None)
    assert discord == 66.66666666666666

    discord = calculate_organization_vote_discord(first_vote, coalition, main_organization, main_organization, coalition)
    assert discord == 95.65217391304348

    discord = calculate_organization_vote_discord(first_vote, last_group, main_organization, main_organization, None)
    assert discord == 95.65217391304348
